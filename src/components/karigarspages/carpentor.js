...
import { Link ,Redirect} from 'react-router-dom'
 ...
 class Carpentor extends Component {
     constructor(props) {
         super(props);
         this.state={
             role:"",
    ...
    render() {
        ...
        const providers=this.props.providers.length!=0?this.props.providers.profiles.filter(item=>item.status=="accepted" && item.category=="carpentor"):""
        return (
            <>
              <ProfilePageHeader/>  
...
const  mapStateToProps = (state) => ({
  ...
  export default connect(mapStateToProps,{FetchProviders})( Carpentor);
