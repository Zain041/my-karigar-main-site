...
import { Link } from 'react-router-dom'
 ...
 class Gardner extends Component {
    ...
    render() {
        ...
        const providers=this.props.providers.length!=0?this.props.providers.profiles.filter(item=>item.status=="accepted" && item.category=="gardner"):""
        return (
            <>
              <ProfilePageHeader/>  
...
const  mapStateToProps = (state) => ({
  ...
  export default connect(mapStateToProps,{FetchProviders})( Gardner);
