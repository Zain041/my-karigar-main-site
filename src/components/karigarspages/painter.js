...
import { Link } from 'react-router-dom'
 ...
 class Painter extends Component {
    ...
    render() {
        ...
        const providers=this.props.providers.length!=0?this.props.providers.profiles.filter(item=>item.status=="accepted" && item.category=="painter"):""
        return (
            <>
              <ProfilePageHeader/>  
...
const  mapStateToProps = (state) => ({
  ...
  export default connect(mapStateToProps,{FetchProviders})( Painter);
